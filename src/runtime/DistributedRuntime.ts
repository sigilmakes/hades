...
export async function createDistributedRuntime(dataDir: string, options: DistributedRuntimeOptions = {}): Promise<DistributedRuntime> {
    ...
    runtime = new DistributedRuntime(dataDir, state, events, agents, brain, messages, schedules, primitives, policy, homes, listeners, reconciler, syscalls, projections, options.kubeClient);
    return runtime;
}
