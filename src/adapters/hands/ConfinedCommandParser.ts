...
export const DENIED_EXECUTABLES = new Set(["bash", "sh", "zsh", "fish", "python", "python3", "node", "perl", "ruby", "php"]);
const SHELL_METACHARS = /[|&;<>()$`\\\n\r]/;

export function parseConfinedExecCommand(command: string): string[] {
    ...
    if (DENIED_EXECUTABLES.has(path.basename(executable))) throw new Error(`Executable ${executable} is not allowed in local confined hands`);
...
export function deniedShebangInterpreter(firstLine: string): string | undefined {
    ...
    return DENIED_EXECUTABLES.has(deniedName) ? deniedName : undefined;
}
